...
import pl.agh.edu.dehasher.backend.{Dehash, DehashWorker}
import pl.agh.edu.dehasher.messages.{Check, FoundIt, RangeChecked, WorkDetails}
    ...
    it("Worker should send FoundIt or RangeChecked message") {
      ...
      val hashes: TableFor6[String, String, String, String, Inclusive[BigInt], Any] =
        Table(
          ...
          ("0efdc6151ea756af355d3fc133f4e8b2145c70a2", "alicja", "SHA-1", a_z, lalaRange to aaaaaRange, RangeChecked(lalaRange to aaaaaRange, WorkDetails("0efdc6151ea756af355d3fc133f4e8b2145c70a2", "SHA-1", a_z))),
      ...
      forAll(hashes) { (hash, dehashed, algo, alphabet, range, message) =>
        ...
        val worker = system.actorOf(Props[DehashWorker])

        When("Check message is send")
        worker ! Check(range, WorkDetails(hash, algo, a_z))
